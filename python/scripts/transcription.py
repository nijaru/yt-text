...
class Transcriber:
    def __init__(
        ...
        # Cache configuration
        self.enable_cache = enable_cache and bool(os.environ.get("ENABLE_CHUNK_CACHE", "true").lower() == "true")
    ...
    def _manage_cache_size(self) -> None:
        ...
        try:
            ...
            if total_size_gb > self.max_cache_size_gb:
                print(f"Cache size ({total_size_gb:.2f} GB) exceeds max ({self.max_cache_size_gb:.2f} GB), cleaning up...")
                ...
                for file_path, size, _ in cache_files:
                    ...
                    try:
                        ...
                        print(f"Removed {file_path.name} ({size_gb:.2f} GB) from cache")
                    except Exception as e:
                        print(f"Failed to remove cache file {file_path}: {e}")
                
                print(f"Cache cleanup complete. New size: approximately {current_size:.2f} GB")
        except Exception as e:
            print(f"Error during cache management: {e}")
    ...
    def _monitor_download_progress(self, progress):
        ...
        if progress['status'] == 'downloading':
            ...
            print(f"Download error: {progress.get('error')}")
            ...
            if self._should_chunk_audio(duration):
                ...
                class AudioStreamContext:
                    ...
                    def __enter__(self):
                        ...
                        print(f"Starting streaming transcription of {self.audio_path}")
                        # Load in 5MB chunks to avoid memory issues
                        self.segment = AudioSegment.from_file(self.audio_path, chunk_size=5*1024*1024)
                        return self
                    ...
                    def __exit__(self, exc_type, exc_val, exc_tb):
                        ...
                        for chunk_path in self.temp_chunks:
                            try:
                                ...
                                print(f"Warning: Failed to remove temp chunk {chunk_path}: {e}")
                ...
                with AudioStreamContext(audio_path) as stream:
                    ...
                    chunk_count = math.ceil(len(stream.segment) / chunk_length_ms)
                    print(f"Processing audio in {chunk_count} chunks of {self.chunk_length_seconds}s each")
                    ...
                    for i, chunk in enumerate(audio_chunks):
                        ...
                        if cached_result:
                            # Cache hit
                            print(f"Cache hit for chunk {i+1}/{len(audio_chunks)}")
                            cache_hits += 1
                            ...
                            print(f"Processing chunk {i+1}/{len(audio_chunks)} ({progress:.0%} complete)")
                            continue
                        ...
                        try:
                            ...
                            if self.enable_cache and chunk_text:
                                cache_data = {
                                    ...
                                    "language_probability": language_info.language_probability if language_info else 0,
                                    "timestamp": time.time()
                                }
                                cache_path = self._save_to_cache(video_id, i, cache_data)
                                if cache_path:
                                    print(f"Saved chunk {i} to cache: {os.path.basename(cache_path)}")
                            ...
                            try:
                                ...
                                print(f"Warning: Failed to remove temp chunk early: {e}")
                ...
                if cached_result:
                    ...
                    print("Cache hit for entire audio file")
                    cache_hits += 1
                    ...
                    if self.enable_cache and text:
                        ...
                        if cache_path:
                            print(f"Saved entire file to cache: {os.path.basename(cache_path)}")
