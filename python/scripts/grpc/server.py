    ...
    def Transcribe(self, request, context):
        ...
        try:
            ...
            for progress, status in self._transcribe_with_progress(request.url, options):
                ...
                if (progress - last_progress >= 0.05 or 
            ...
            if hasattr(thread_data, 'final_result') and thread_data.final_result:
                ...
                logger.info(f"Using cached transcription result (streaming optimization)")
            elif saw_error:
                ...
                logger.warning("Falling back to separate transcription call - suboptimal performance")
    ...
    def _transcribe_with_progress(self, url: str, options: Dict[str, str]) -> Iterator[tuple[float, str]]:
        ...
        def download_progress_callback(progress):
            if progress['status'] == 'downloading':
                ...
                progress_tracker.set_error(f"Download error: {progress.get('error', 'unknown error')}")
        ...
        transcription_thread = threading.Thread(
            ...
            args=(url, options, progress_tracker, download_progress_callback, chunk_progress_callback)
        )
    ...
    def _run_transcription_thread(
        ...
        try:
            ...
            def wrapped_transcribe(audio_path, duration):
                ...
                # Apply our wrapper
                if '_should_chunk_audio' in dir(transcriber) and transcriber._should_chunk_audio(duration):
            ...
            logger.error(f"Exception in transcription thread: {e}\n{error_details}")
